...
export const getFavoriteProducts = async (req: Request, res: Response) => {
    try {
        ...
        const favoriteProducts = await FavoriteProducts.findOne({ where: { User: user }, relations: ['Products'] });
        if (!favoriteProducts) {
...
export const addFavoriteProduct = async (req: Request, res: Response) => {
    try {
        ...
        const user = await Users.findOne({ where: { UserID: cookie } });
        if (!user) {
            return res.status(404).json({ message: 'You must login first!' });
        }
        ...
        const isFavorite = await FavoriteProducts.findOne({ where: { User: user, Product: product } });
        if (isFavorite) {
...
export const removeFavoriteProduct = async (req: Request, res: Response) => {
    try {
        ...
        const favoriteProduct = await FavoriteProducts.findOne({ where: { User: user, Product: product } });
        if (!favoriteProduct) {
