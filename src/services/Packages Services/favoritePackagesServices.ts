...
export const s_getAllFavoritePackagesUser = async (req: Request, res: Response) => {
        try {
                ...
                const getAllFavoritePackages = await FavoritePackages.find({ where: { User: { UserID: userId } } });
                if (getAllFavoritePackages.length === 0) {
                        return res.status(404).send({ message: "No favorite packages found" });
                }
