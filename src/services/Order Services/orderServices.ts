...
export const s_checkoutOrderUser = async (req: Request, res: Response) => {
    try {
        ...
        const paymentMethod = await PaymentMethods.findOne({ where: { PaymentMethodID: PaymentMethodID, User: user } });
        if (!paymentMethod) {
            return res.status(404).send({ message: "Payment method not found" });
        }

        const address = await Addresses.findOne({ where: { AddressID: AddressID, User: user } });
        if (!address) {
